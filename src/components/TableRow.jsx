...
const TableRow = ({ item, index }) => (
    <tr className={`max-sm:grid gap-1 ${index % 2 === 0 ? "bg-backgound-color-secondary" : ""} p-5`}>
        <td className=" pl-2 pr-10 relative max-sm:before:content-['Navn:'] before:font-bold before:underline-offset-4 before:underline before:text-md before:absolute before:inset-0 max-sm:pt-[30px] max-sm:pb-3"> {capitalizeFirstLetter(item.name)}</td>
        <td className=" pl-2 pr-10 relative max-w-[70ch] max-sm:before:content-['Beskrivelse:'] before:font-bold before:text-md before:absolute before:inset-0 max-sm:pt-[30px] max-sm:pb-3 before:underline-offset-4 before:underline"> {capitalizeFirstLetter(item.shortDesc)}</td>
